...
import { isOwnerRelation, NamingHelper } from '@contember/schema-utils'
...
export const convertError = (e: any): MutationResult => {
	...
	if (e instanceof Database.UniqueViolationError) {
		...
		const path = constraint ? [{ field: constraint.fields[0] }] : []
		return new MutationConstraintViolationError(path, ConstraintType.uniqueKey, message, [MutationResultHint.sqlError])
	}
